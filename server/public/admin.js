...
// 2. Authentication: Google Admin Sign-In
window.submitAdminGoogleLogin = async function() {
  const errorMsg = document.getElementById("firebaseAuthErrorMsg");
  const googleBtn = document.getElementById("btnAdminGoogleLogin");

  if (!auth) {
    if (errorMsg) errorMsg.innerText = "Firebase Auth not ready. Please refresh the page.";
    return;
  }
  ...
  try {
    ...
    if (!user) throw new Error("No user returned from Google authentication.");

    const idToken = await user.getIdToken(true);

    // Verify token and admin IAM privileges on server
    ...
    if (res.ok && data.authenticated) {
      ...
      if (errorMsg) {
        errorMsg.innerText = data.error || `Access Denied: ${user.email} is not authorized as an administrator.`;
      }
    ...
    if (err.code === "auth/popup-closed-by-user") {
      if (errorMsg) errorMsg.innerText = "Google sign-in popup was closed.";
    } else if (err.code === "auth/popup-blocked") {
      if (errorMsg) errorMsg.innerText = "Popup was blocked by browser. Please allow popups.";
    } else {
      if (errorMsg) errorMsg.innerText = "Google sign-in error: " + err.message;
    }
  } finally {
    if (googleBtn) {
...
// 2b. Authentication: Firebase Email & Password Login
